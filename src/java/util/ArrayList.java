...
public class ArrayList<E> extends AbstractList<E> implements List<E>, RandomAccess, Cloneable, java.io.Serializable {
     ...
     * 答：ArrayList在序列化的时候会调用writeObject，直接将size和element写入ObjectOutputStream；反序列化时调用readObject，从
     * ObjectInputStream获取size和element，再恢复到elementData。
     *
     * 【问题2】为什么不直接用elementData来序列化，而采用上述的方式来实现序列化呢？
     * 答：原因在于elementData是一个缓存数组，它通常会预留一些容量，等容量不足时再扩充容量，那么有些空间可能就没有实际存储元素，采用上述的方式来
     * 实现序列化时，就可以保证只序列化实际存储的那些元素，而不是整个数组，从而节省空间和时间。
     */
    ...
    /**
     * 构造一个空容量的数组（DEFAULTCAPACITY_EMPTY_ELEMENTDATA = {}）
     */
    public ArrayList() {
    ...
    /**
     * 计算ArrayList的容量，如果elementData数组中没有已存储的元素，则返回默认值10，否则，返回minCapacity。
     */
    // eg1：第一次新增元素，elementData={} minCapacity=1
    private static int calculateCapacity(Object[] elementData, int minCapacity) {
        if (elementData == DEFAULTCAPACITY_EMPTY_ELEMENTDATA)
            return Math.max(DEFAULT_CAPACITY, minCapacity); // eg1：return 10
    ...
    private void ensureExplicitCapacity(int minCapacity) {
        /** 变化次数加1 */
        modCount++; // eg1: modCount++后，modCount=1

        /** 如果所需的【最小容量】大于elementData数组的容量，则进行扩容操作 */
        if (minCapacity - elementData.length > 0)  // eg1：elementData.length=0
            grow(minCapacity); // eg1：minCapacity=10
    }
    ...
    private void grow(int minCapacity) {
        ...
        if (newCapacity - minCapacity < 0)
            newCapacity = minCapacity; // eg1：newCapacity=10
        ...
        elementData = Arrays.copyOf(elementData, newCapacity); // eg1：newCapacity=10， 扩容elementData的length=10
    }
    ...
    public boolean add(E e) {
        ...
        ensureCapacityInternal(size + 1); // eg1：size=0
    ...
    public E remove(int index) {
        ...
        E oldValue = elementData(index); // eg1：oldValue="a1"
        ...
        if (numMoved > 0) {
             ...
             * {"a1","a2","a3","a4"}
             *
             * 假设删除"a2"，那么index=1, numMoved=4-1-1=2, 那么下面的语句执行的就是：
